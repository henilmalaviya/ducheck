...
async function checkDiskSpace() {
  try {
    ...
    console.log(`[${new Date().toISOString()}] Disk usage for ${TARGET_MOUNT_POINT}: ${usePercent}%. Threshold: ${THRESHOLD_PERCENTAGE}%.`);
    ...
    if (usePercent >= THRESHOLD_PERCENTAGE) {
      ...
      if (alertSentFlag) {
        console.log(`[${new Date().toISOString()}] Disk usage dropped below threshold. Alert reset.`);
      }
      alertSentFlag = false;
    }
  } catch (error) {
    console.error(`[${new Date().toISOString()}] Error checking disk space:`, error);
  }
}
...
async function sendAlertEmail(usePercent) {
  ...
  try {
    await mg.messages.create(MG_DOMAIN, {
      from: FROM_EMAIL,
      to: [TO_EMAIL],
      subject: `🚨 CRITICAL: Disk Space Alert on Server (${usePercent}%)`,
      text: `Warning: The disk space on mount point '${TARGET_MOUNT_POINT}' has exceeded the configured threshold of ${THRESHOLD_PERCENTAGE}%.\n\nCurrent usage is at ${usePercent}%.\n\nPlease take immediate action to free up space.`,
    });
...
function start() {
  if (!MG_API_KEY || !MG_DOMAIN || !TO_EMAIL) {
